...
const factory = (env) => {
  ...
  // Bridge a node-style FormData stream (e.g. the `form-data` package) into a
  // web ReadableStream that fetch can consume. transformRequest is a
  // synchronous contract, so this async normalization cannot happen there.
  const streamToWebStream =
      ...
      return new ReadableStream({
        start(controller) {
          stream.on('data', (chunk) => {
            controller.enqueue(utils.isString(chunk) ? encoder.encode(chunk) : chunk);
        ...
        cancel(reason) {
          if (utils.isFunction(stream.destroy)) {
  ...
  return async (config) => {
    ...
    try {
      ...
      // A FormData implementation backed by a node-style stream (e.g. the
      // `form-data` package) cannot be serialized by fetch — it would be
      // stringified to "[object FormData]". Bridge it to a ReadableStream
      // here and reconcile Content-Length and upload progress accounting
      // with the boundary headers resolveConfig already applied.
      if (
        streamToWebStream &&
        ...
        if (!headers.hasContentLength() && utils.isFunction(data.getLength)) {
          try {
            const knownLength = await new Promise((resolve, reject) => {
              data.getLength((err, length) => (err ? reject(err) : resolve(length)));
            });

            if (Number.isFinite(knownLength) && knownLength >= 0) {
              headers.setContentLength(knownLength);
              requestContentLength = knownLength;
            }
            /*eslint no-empty:0*/
          } catch (e) {}
        }

        data = streamToWebStream(data);
      }
