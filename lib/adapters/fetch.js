...
const factory = (env) => {
  ...
  return async (config) => {
    ...
    try {
      ...
      // The `form-data` package (Node's default FormData) produces an async
      // CombinedStream body that fetch cannot consume — undici would stringify
      // it to "[object FormData]". transformRequest is a synchronous contract,
      // so the stream is normalized at this async layer instead, mirroring the
      // http adapter: resolveConfig has already applied the boundary headers
      // from getHeaders(); backfill Content-Length from getLength() when it is
      // knowable, then hand fetch a real ReadableStream so upload progress and
      // maxBodyLength accounting measure the actual multipart bytes.
      if (
        utils.isFormData(data) &&
        utils.isFunction(data.getHeaders) &&
        data.getHeaders !== Object.prototype.getHeaders &&
        platform.toWebStream
      ) {
        ...
        data = platform.toWebStream(data);
      }
